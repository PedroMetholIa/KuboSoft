...
@Component({
  ...
  `,
  styleUrl: './teg-juego.component.scss'
})
export class TegJuegoComponent implements OnInit, OnDestroy {
  ...
  async loadGameState() {
    ...
    // Marcar que este jugador está dentro y verificar si el juego puede arrancar
    if (this.miJugador) {
  ...
  private async reloadJugadores() {
    ...
    // Solo el host arranca el juego cuando todos están dentro
    if (
      this.esHost &&
      this.todosAdentro &&
  ...
  subscribeRealtime() {
    this.channelPartida = this.service.client
      .channel(`teg-partida-${this.partidaId}`)
    ...
    this.channelTurno = this.service.client
      .channel(`teg-turno-${this.partidaId}`)
